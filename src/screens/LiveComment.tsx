...
import { ActivityIndicator, Banner, IconButton, Surface, Text } from 'react-native-paper'
import ky from 'ky'

import { DefaultLiveAvatar } from '../assets'
import i18n from '../locales/i18n'
import { SettingsContext } from '../context/SettingsContext'
import { LiveCommentNode } from '../types'
 ...
 * @version 2.0
 */
export default function LiveCommentScreen({ onRefresh }: { onRefresh: () => void }) {
  ...
  const extractLiveContent = (node: any, liveContents: any) => {
    // recursive call
  ...
  const init = async () => {
    ...
    const posts = settingsContext.doc.querySelector('#post-container')
    let c = []
    let id = null
  ...
  const renderLiveContents = (liveContents: any[]) => {
    let contents = []
    for (const i in liveContents) {
      ...
      switch (lc.type) {
        ...
        case 'paragraph':
          ...
          contents.push(
            <Text variant="bodyMedium" key={i} style={s}>
              {lc.text}
            </Text>
          )
          break
    ...
    for (const i in comments) {
      ...
      c.push(
        <View key={'header-' + i} style={styles.headerContainer}>
          ...
          <Text variant="bodySmall">{comment.authorDate}</Text>
        </View>
      )
      comment.tagText &&
        c.push(

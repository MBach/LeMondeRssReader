...
import { ArticleType, ParsedRssItem } from '../types'
...
export default function HomeScreen() {
  ...
  useEffect(() => {
    fetchFeed(false, route.params?.uri)
  }, [route.params?.uri])

  const refreshFeed = async () => {
    setRefreshing(true)
    await fetchFeed(true, route.params?.uri)
    setRefreshing(false)
  }

  const fetchFeed = async (isRefreshing: boolean, uri?: string) => {
    if (!isRefreshing) {
      setLoading(true)
    }
  ...
  return (
    <Surface
      ...
      <Appbar.Header style={{ marginTop: StatusBar.currentHeight }}>
        ...
        <Appbar.Content title={route.params?.title ?? i18n.t('feeds.latestNews')} />
      </Appbar.Header>
      {loading ? (
        renderContentLoader()
      ) : (
